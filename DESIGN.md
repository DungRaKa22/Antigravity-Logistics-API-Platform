---
version: alpha
name: Antigravity-Express-Design-System
description: A minimal black-and-white design system for Antigravity Express logistics platform, framed by a custom geometric display sans (Inter), accented by a signature pill shape (radius 999px), and tailored for professional transportation and delivery UI.
...
1. **A custom geometric display sans** (extracted as `Inter`) for every headline. Weight 700 only; no italic; no tracking variation. Sizes range from `display-sm` 20 px up to `display-xxl` 52 px on the hero. Line-heights tighten to 1.22 – 1.25 at display sizes for a poured-on-the-page look.
2. **A custom text sans** (extracted as `InterText`) for body, button, link, and small headings. Weights 400 and 500 are the working pair. Used at 12 – 18 px; 24 px maximum for ride-request form labels. Tracking is always neutral.
...
- **Two faces, two roles.** Inter for display; InterText for everything else. Never cross the streams.
